...
export async function POST(req, { params }) {
  ...
  const oldPermit = await prisma.familyPermit.findUnique({
    where: { id: permitId },
    include: { permitType: true },
  });
  if (!oldPermit || oldPermit.familyId !== familyId)
  ...
  await prisma.familyPermit.update({
    where: { id: permitId },
    data: { status: "replaced" },
  });

  const newPermit = await prisma.familyPermit.create({
    data: {
      familyId,
